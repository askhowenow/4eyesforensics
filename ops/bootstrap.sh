...
metadata_token=$(curl -fsS -X PUT --max-time 2 -H "X-aws-ec2-metadata-token-ttl-seconds: 21600" http://169.254.169.254/latest/api/token || true)
public_ip=$(curl -fsS --max-time 2 -H "X-aws-ec2-metadata-token: ${metadata_token}" http://169.254.169.254/latest/meta-data/public-ipv4 || true)
if [ -n "${public_ip}" ]; then
